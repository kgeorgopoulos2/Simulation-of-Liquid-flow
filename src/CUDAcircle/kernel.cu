...
//kernel to find the point position
__global__ void findPointsKernel(Point *p, int *pInd, int sizePoint, Circle *c, int sizeCircle, bool *d_Change)
{
    ...
    if(tid < sizePoint){
		...
		//Calculate the error from the point to every circle
		for(unsigned int i=0; i<sizeCircle; ++i){
			Circle cc = c[i];
			float xa = pp.x - cc.x;
			float yb = pp.y - cc.y;
			float r = xa * xa + yb * yb - cc.r * cc.r;
			r = r * r;
			if(r < minErr){
				minErr = r;
				pos = i;
		...
		//Set the new position with less error
		if(pInd[tid] != pos){
			pInd[tid] = pos;
			d_Change[0] = true;
		}
...
void findPoints(thrust::device_vector<Point> & d_ev, thrust::device_vector<int> &d_points, thrust::device_vector<Circle> &d_circles, int &numChanges){
	...
	bool Change = false, * d_Change; //This variable stores if there are any changes in the points 
	...
	//Call to the kernel
	findPointsKernel<<<(d_ev.size() + 64)/64, 64>>>(thrust::raw_pointer_cast( &d_ev[0]), thrust::raw_pointer_cast( &d_points[0]), d_ev.size(),
														thrust::raw_pointer_cast( &d_circles[0]), d_circles.size(), d_Change);

	//Read the value from GPU memory
...
void  fitCircles(thrust::device_vector<Point> & ev, int K, thrust::device_vector<int> &points, thrust::device_vector<Circle> &circles){
	...
	for(int i=0; i<K; ++i){
		//calculate the points corresponding to circle "i" in parallel
...
//Struct to store the acumulation results
struct ResultStruct{
	float x, y, xx, yy, xy, aux1, aux2;
};

//Some binary operation for the acumulation results
 struct binaryOperatorResult2 : public thrust::binary_function<ResultStruct,ResultStruct,ResultStruct>
{
 ...
 //Some unary operation for the acumulation result
...
void fitring(thrust::device_vector<Point> &points, Circle &circ){
	...
	//Acumulate the results in res to calculate the matrix in a parallel way
	...
	//Resolve Ax=b => A'Ax=A'b, where A'is the transpose matrix and A'A is a 3x3 matrix
...
int main(int argc, char *argv[]){
    ...
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "CUDA function failed!");
        return 1;
    }
